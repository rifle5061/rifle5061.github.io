...
def fetch_url(url: str, timeout: int = 12) -> str | None:
    headers = {
        "User-Agent": "Mozilla/5.0 (compatible; BoatAIRiskLab/0.9; +https://rifle5061.github.io/boat-ai-risk-lab/)"
    }
    last_error = None
...
def target_jcds_from_official_index(date: str) -> list[str]:
    urls = [f"{BASE_INDEX_URL}?hd={date}", BASE_INDEX_URL]
    for url in urls:
        ...
        if found:
            log(f"[INFO] target venues from official index: {', '.join(JCD_MAP[x] for x in found)}")
            return found
        text = BeautifulSoup(html, "html.parser").get_text("\n")
        places_found = sorted({jcd for jcd, place in JCD_MAP.items() if place in text})
        if places_found:
            log(f"[INFO] target venues from official index text: {', '.join(JCD_MAP[x] for x in places_found)}")
            return places_found
    return []
...
def is_noise_name_line(line: str) -> bool:
    if not line:
        return True
    if "/" in line:
        return True
    if line in BRANCHES:
        return True
    if line in ["全国", "当地", "モーター", "ボート", "勝率", "2連率", "3連率", "写真", "氏名", "枠", "級別", "平均ST"]:
        return True
    if re.fullmatch(r"(F|L)\d+", line):
        return True
...
def is_japanese_fragment(line: str) -> bool:
    if is_noise_name_line(line):
        return False
    return bool(re.fullmatch(r"[一-龥ぁ-んァ-ンー・]+", line))


def find_name(block: list[str], cls_idx: int | None) -> str:
    start = (cls_idx + 1) if cls_idx is not None else 0
    fragments: list[str] = []
    for line in block[start:start + 18]:
        if "/" in line and any(b in line for b in BRANCHES):
            break
        if re.fullmatch(r"(F|L)\d+", line):
            break
        if is_japanese_fragment(line) and len(line) <= 8:
    ...
    if len(fragments) == 1:
        return fragments[0]
    joined = "\n".join(block)
    m = re.search(r"([一-龥ぁ-んァ-ンー・]{1,5})\s+([一-龥ぁ-んァ-ンー・]{1,5})", joined)
    if m:
        cand = f"{m.group(1)} {m.group(2)}"
        if not any(noise in cand for noise in ["全国", "当地", "モーター", "ボート"]):
            return cand
    return ""
...
def parse_block(frame: int, block: list[str]) -> dict[str, Any]:
    ...
    for i, line in enumerate(block):
        ...
        if m:
            ...
            cls_idx = i
            break
    e["racer_name"] = find_name(block, cls_idx)
    for line in block:
        if "/" in line:
    ...
    nums: list[str] = []
    for i, line in enumerate(block):
        if re.search(r"(?<![0-9])(?:0\.\d{2}|\.\d{2})(?![0-9])", line):
            avg_idx = i
            break
    if avg_idx is not None:
        for line in block[avg_idx:]:
            nums.extend(decimals(line))
            if len(nums) >= 11:
                break
    if len(nums) >= 1:
        e["avg_st"] = nums[0]
...
def parse_entries(soup: BeautifulSoup) -> list[dict[str, Any]]:
    ...
    seen = set()
    for frame, idx in frame_indexes:
        if frame not in seen:
            dedup.append((frame, idx))
            seen.add(frame)
    entries: list[dict[str, Any]] = []
    for idx, (frame, start_i) in enumerate(dedup):
        end_i = dedup[idx + 1][1] if idx + 1 < len(dedup) else len(lines)
        entries.append(parse_block(frame, lines[start_i:end_i]))
...
def analyze(entries: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for i, e in enumerate(entries, start=1):
        ...
        risks.append({"boat": i, "role": roles[i], "rank": rank, "risk_note": f"{roles[i]}として評価。平均ST・選手成績・当地・モーター/ボートを確認。"})
...
def parse_race(date: str, jcd: str, rno: int, html: str) -> dict[str, Any] | None:
    ...
    return {
        ...
        "distance": "1800m",
        "status": "分析済み",
        "detail_level": "github_actions_parallel_parser_v1",
        "entries": entries,
        **analysis,
        "summary": summary,
...
def fetch_and_parse(task: tuple[str, str, int]) -> tuple[dict[str, Any] | None, str]:
    ...
    return item, f"[OK] {place} {rno}R names={names} deadline={item.get('deadline','')}"
