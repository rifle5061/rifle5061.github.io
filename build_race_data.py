...
def analyze(entries: list[dict[str, Any]]) -> dict[str, Any]:
    """
    サイト用の詳細リスクデータ生成版。
    出走表データだけで、1号艇信頼度・荒れ度・艇別リスク・危険人気候補・買い目候補を自動生成する。
    オッズ・展示後補正は未取得のため、人気ズレや展示補正は「未反映」として扱う。
    """
    roles = {
        1: "イン信頼度",
        2: "差し脅威度＋壁性能",
    ...
    def st_value(e: dict[str, Any], default: float = 0.18) -> float:
        raw = str(e.get("avg_st", "")).strip()
        if raw == "-":
            return default
    ...
    def boat_score(e: dict[str, Any], boat: int) -> float:
        n3 = to_float(e.get("national_3rate"), 45)
        l3 = to_float(e.get("local_3rate"), 45)
        m2 = to_float(e.get("motor_2rate"), 30)
        m3 = to_float(e.get("motor_3rate"), 40)
        b3 = to_float(e.get("boat_3rate"), 40)
        st = st_value(e)

        frame_bonus = {1: 16, 2: 11, 3: 7, 4: 4, 5: 0, 6: -4}.get(boat, 0)
        st_bonus = (0.18 - st) * 120
    ...
    for i, e in enumerate(entries, start=1):
        scores[i] = boat_score(e, i)

    e1 = entries[0] if entries else {}
    st1 = st_value(e1, 0.17)
    n3_1 = to_float(e1.get("national_3rate"), 50)
    l3_1 = to_float(e1.get("local_3rate"), 50)
    m2_1 = to_float(e1.get("motor_2rate"), 35)
    m3_1 = to_float(e1.get("motor_3rate"), 45)
    b3_1 = to_float(e1.get("boat_3rate"), 45)
    ...
    outside_pressure = max([scores.get(2, 50), scores.get(3, 50), scores.get(4, 50)] or [50])
    if outside_pressure >= 75:
        trust -= 6
    elif outside_pressure >= 68:
        trust -= 3

    if m2_1 < 25 or m3_1 < 35:
        trust -= 8
    elif m2_1 < 30:
        trust -= 4

    trust = clamp(trust, 35, 92)

    rough = 100 - trust
    rough += max(0, outside_pressure - 65) * 0.35
    if m2_1 < 28 or m3_1 < 38:
        rough += 8
    rough = clamp(rough, 15, 90)

    motor_overvalue_risk = 0
    for boat, e in enumerate(entries, start=1):
        m2 = to_float(e.get("motor_2rate"), 0)
        m3 = to_float(e.get("motor_3rate"), 0)
        n3 = to_float(e.get("national_3rate"), 0)
        st = st_value(e, 0.18)
        if (m2 >= 45 or m3 >= 55) and (boat >= 4 or n3 < 45 or st >= 0.18):
            motor_overvalue_risk += 12
    motor_overvalue_risk = clamp(motor_overvalue_risk, 0, 100)

    st_trap = 0
    st_values = {i: st_value(e, 0.18) for i, e in enumerate(entries, start=1)}
    for boat in range(2, 7):
        own = st_values.get(boat, 0.18)
        inner = st_values.get(boat - 1, 0.18)
        diff = inner - own
        if own <= 0.15 and diff < 0.03:
            st_trap += 8
        if boat >= 5 and own <= 0.15:
            st_trap += 5
    average_st_trap_index = clamp(st_trap, 0, 100)

    wall_2 = scores.get(2, 50)
    attack_3 = scores.get(3, 50)
    attack_4 = scores.get(4, 50)
    collapse = 25
    if trust < 65:
        collapse += 18
    if wall_2 < 55:
    ...
    if attack_4 >= 68:
        collapse += 9
    if max(scores.get(5, 45), scores.get(6, 40)) >= 62:
        collapse += 6
    development_collapse_risk = clamp(collapse, 10, 95)

    skip_index = clamp((rough * 0.45 + development_collapse_risk * 0.35 + motor_overvalue_risk * 0.20), 0, 100)

    risks = []
    for i, e in enumerate(entries, start=1):
        score = scores.get(i, 45)
        rank = rank_from_score(score)

        n3 = to_float(e.get("national_3rate"), 0)
        l3 = to_float(e.get("local_3rate"), 0)
        m2 = to_float(e.get("motor_2rate"), 0)
        m3 = to_float(e.get("motor_3rate"), 0)
        b3 = to_float(e.get("boat_3rate"), 0)
        st = str(e.get("avg_st", "-"))

        if i == 1:
            note = f"1号艇信頼度{trust}。平均ST{st}、全国3連率{n3:.2f}、当地3連率{l3:.2f}、M2/M3={m2:.2f}/{m3:.2f}で評価。2〜4号艇の攻撃圧も加味。"
        elif i == 2:
            note = f"差し脅威度と壁性能を評価。平均ST{st}、M2/M3={m2:.2f}/{m3:.2f}。1号艇を守れるか、3号艇の攻めを止められるかが焦点。"
        elif i == 3:
            note = f"攻撃脅威度＋3着残り評価。平均ST{st}、全国/当地3連率={n3:.2f}/{l3:.2f}、M3={m3:.2f}。まくり差し・内残りを確認。"
        elif i == 4:
            note = f"カド攻撃度評価。平均ST{st}、M2/M3={m2:.2f}/{m3:.2f}。自力攻めか、3号艇の攻めに乗る展開待ちかを確認。"
        elif i == 5:
            note = f"展開突入度評価。平均ST{st}、M3={m3:.2f}、B3={b3:.2f}。勝ち切りより2・3着の展開突入候補。"
        else:
            note = f"穴絡み度評価。平均ST{st}、M3={m3:.2f}、B3={b3:.2f}。基本は不利だが隊形崩壊時の3着穴を確認。"
    ...
    top_sorted = sorted(risks, key=lambda x: (-x["score"], x["boat"]))
    top4 = [
        {
            "boat": x["boat"],
            "label": x["role"],
    ...
    if trust < 65:
        danger.append({
            "boat": 1,
            "reason": f"1号艇信頼度{trust}で過信注意。イン人気でも、モーター・ST・2〜4号艇の攻撃圧を確認。"
        })
    ...
    for x in top_sorted:
        ...
        m2 = to_float(e.get("motor_2rate"), 0)
        m3 = to_float(e.get("motor_3rate"), 0)
        if x["score"] >= 68 and boat >= 5:
            danger.append({
                "boat": boat,
                "reason": f"{boat}号艇はデータ評価は高めだが外枠不利あり。人気するなら過信注意。M2/M3={m2:.2f}/{m3:.2f}、平均ST{e.get('avg_st','-')}。"
            })
        elif x["score"] >= 70 and st >= 0.18:
            danger.append({
                "boat": boat,
                "reason": f"{boat}号艇は総合評価は高いが平均STが遅め。人気するならスタート隊形確認。"
            })
        if len(danger) >= 3:
            break
    ...
    if not danger:
        danger.append({
            "boat": top_sorted[0]["boat"] if top_sorted else 1,
            "reason": "オッズ未取得のため人気ズレは未反映。人気順だけでなくST・モーター・枠のズレを確認。"
        })

    top_boats = [x["boat"] for x in top_sorted]
    a = top_boats[0] if top_boats else 1
    b = top_boats[1] if len(top_boats) > 1 else 2
    c = top_boats[2] if len(top_boats) > 2 else 3
    d = top_boats[3] if len(top_boats) > 3 else 4

    if trust >= 72:
        main = [f"1-{b}-{c}", f"1-{c}-{b}", f"1-{b}-{d}", f"1-{d}-{b}", f"1-{c}-{d}"]
        osa = [f"{b}-1-{c}", f"1-5-{b}", f"{c}-1-{b}"]
    elif trust >= 62:
        main = [f"1-{b}-{c}", f"1-{c}-{b}", f"{b}-1-{c}", f"1-{b}-{d}", f"{c}-1-{b}"]
        osa = [f"{b}-{c}-1", f"{c}-{b}-1", f"1-5-{c}"]
    else:
        attack = a if a != 1 else b
        second = b if b != attack else c
        main = [f"{attack}-1-{second}", f"1-{attack}-{second}", f"{attack}-{second}-1", f"1-{second}-{attack}", f"{second}-1-{attack}"]
        osa = [f"4-1-5", f"5-1-{attack}", f"{attack}-5-1"]

    def valid_ticket(t: str) -> bool:
        parts = t.split("-")
        return len(parts) == 3 and len(set(parts)) == 3 and all(p in {"1","2","3","4","5","6"} for p in parts)

    def uniq_tickets(tickets: list[str], limit: int) -> list[str]:
        out = []
        for t in tickets:
            ...
            if len(out) >= limit:
                break
        fillers = ["1-2-3","1-3-2","1-2-4","1-4-2","2-1-3","3-1-2","1-5-3","2-3-1"]
        for t in fillers:
            if valid_ticket(t) and t not in out:
                out.append(t)
            if len(out) >= limit:
    ...
    main = uniq_tickets(main, 5)
    osa = uniq_tickets(osa, 3)
    ...
    if trust >= 72:
        ...
        risk_level = "B-（1号艇過信注意）"

    summary = (
        f"1号艇信頼度{trust}、荒れ度{rough}、見送り指数{skip_index}。"
        f"展開崩壊リスク{development_collapse_risk}、モーター過大評価リスク{motor_overvalue_risk}。"
        "オッズ・展示後補正は未反映のため、直前情報と人気順で最終確認。"
    )

    analysis_text = (
        "出走表データから自動生成したサイト用詳細リスク。"
        f"平均STトラップ指数{average_st_trap_index}、"
        f"展開崩壊リスク{development_collapse_risk}、"
        f"見送り指数{skip_index}。"
        "オッズ取得前のため、危険人気は『人気した場合の注意候補』として扱う。"
    )
    ...
    return {
        ...
        "motor_overvalue_risk": motor_overvalue_risk,
        "average_st_trap_index": average_st_trap_index,
        "development_collapse_risk": development_collapse_risk,
...
def parse_race(date: str, jcd: str, rno: int, html: str) -> dict[str, Any] | None:
    ...
    summary = analysis.get("summary", f"{place}{rno}Rは、1号艇信頼度{analysis['first_boat_reliability']}、荒れ度{analysis['roughness']}。AI上位4艇と艇別リスクを確認。")
    ...
    return {
        ...
        "distance": "1800m",
        "status": "分析済み",
        "detail_level": "github_actions_detailed_risk_v1",
        "entries": entries,
        "_source_snippet": extract_source_snippet(soup),
        **analysis,
        "summary": summary,
        "analysis_text": analysis.get("analysis_text", f"GitHub Actionsで公式開催一覧から開催場を自動判定し、出走表主要データを取得。{summary}"),
